...
namespace CipherChallenge
{
	...
	public class CipherChallenge
	{
	  ...
	  public String MartianCipher(String codedMessage)
	  {
	    ...
	    {
	      String.Concat(decodedSentence, letter);
	    }

	    Regex regex = new Regex(@"\d+"); //What is this matching? Look at the regex documentation for help.

	    if (regex.IsMatch(decodedSentence))
	    {
	      Match match = regex.Match(decodedSentence);
